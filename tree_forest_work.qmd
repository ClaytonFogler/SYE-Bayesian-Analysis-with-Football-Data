...
predictions <- foreach(
  ...
  .combine = cbind
    ...
    as.numeric(predict(bagged_tree, newdata = all_seasons) [, 2])
}
...
predictions <- foreach(i = 1:160, 
  ...
  predict(bagged_tree, newdata = all_seasons)[, 2]
}
...
```{r}
all_seasons$SUCCESSFUL <- as.numeric(all_seasons$SUCCESSFUL)
...
*random forest didn't like my column name so I changed it to get rid of the /*


```{r}
all_seasons_rename <- all_seasons %>%
  rename(
...
```{r}
# number of features
n_features <- length(setdiff(names(all_seasons_rename), "SUCCESSFUL"))

# train a default random forest model
football_rf1 <- ranger(
  SUCCESSFUL ~ ., 
  data = all_seasons_rename,
